...
const RADIUS = 0.35; // miles - check stations within this radius
const allNearbyStations = new Set();

console.log("=== VENUES WITH MULTIPLE NEARBY STATIONS (within 0.35mi) ===\n");
...
for (const venue of venues) {
  ...
  for (const station of uniqueStations) {
    ...
    if (dist <= RADIUS) {
      ...
      allNearbyStations.add(station.name);
    }
  }

  if (nearby.length > 1) {
    nearby.sort((a, b) => a.distance - b.distance);
    console.log(`${venue.name} (${venue.hood}) - ${nearby.length} stations:`);
    for (const s of nearby) {
      console.log(`  • ${s.name} [${s.routes}] - ${s.distance.toFixed(2)} mi`);
    }
    console.log();
  }
}

console.log("\n=== ALL UNIQUE STATIONS WITHIN 0.35mi OF ANY VENUE ===");
console.log(`Total: ${allNearbyStations.size} stations\n`);

// Output as JSON for easy use
const stationList = [];
for (const name of allNearbyStations) {
  const s = uniqueStations.find(st => st.name === name);
  stationList.push({ name: s.name, routes: s.routes, lat: s.lat, lng: s.lng });
}
stationList.sort((a, b) => a.name.localeCompare(b.name));

console.log("Origin clusters JSON:");
console.log(JSON.stringify(stationList, null, 2));
