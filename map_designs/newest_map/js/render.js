...
function render(mode) {
    ...
    if (drawerTitleEl) {
        if (isCurrentPlanningDay) {
            drawerTitleEl.textContent = 'Your Night';
        } else if (mode === 'tomorrow') {
            drawerTitleEl.textContent = "Tomorrow's Lineup";
        } else if (activeDayName) {
            drawerTitleEl.textContent = `${activeDayName}'s Lineup`;
        }
    }
    ...
    if (subtitleEl) {
        ...
        if (filtered.length === 0) {
        ...
        } else if (isCurrentPlanningDay) {
            // Find the best upcoming mic (soonest start, not already live)
            const upcoming = filtered
                .filter(m => m.status === 'upcoming' && m.start)
            ...
            if (hasRoute) {
                subtitleEl.textContent = `${STATE.route.length} Stop${STATE.route.length === 1 ? '' : 's'} Planned`;
                subtitleEl.classList.remove('cta');
            } else if (live.length > 0) {
                subtitleEl.textContent = 'Pick Your Mics For Tonight';
                subtitleEl.classList.add('cta');
            } else if (upcoming.length > 0) {
                const next = upcoming[0];
                const minsUntil = Math.ceil((next.start - currentTime) / 60000);
                const venue = next.venue || next.name || '';
                subtitleEl.textContent = `${venue} Starts In ${minsUntil} Min`;
                subtitleEl.classList.remove('cta');
            } else {
                subtitleEl.textContent = 'Pick Your Mics For Tonight';
                subtitleEl.classList.add('cta');
            ...
            const dayLabel = mode === 'tomorrow' ? 'Tomorrow' : activeDayName || '';
            subtitleEl.textContent = `${filtered.length} Mics ${dayLabel}`;
            subtitleEl.classList.remove('cta');
        }
    }
